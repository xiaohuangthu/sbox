...
//int main()
{
	...
	int s2[SIZE] = { 0 }, s3[SIZE] = { 0 },  s4[SIZE] = { 0 };
	...
	for (ii = 0; ii < test; ii++)
	{
		...
		while (flag == 0)
		{
			...
			if (ddt[1 ^ 2][ar[1] ^ ar[2]] != 0 & ddt[1 ^ 3][ar[1] ^ ar[3]] != 0 & ddt[1 ^ 4][ar[1] ^ ar[4]] != 0)
			{
				if(ddt[2 ^ 3][ar[2] ^ ar[3]] != 0 & ddt[2 ^ 4][ar[2] ^ ar[4]] != 0)
				{
					if(ddt[3^4][ar[3]^ar[4]]!=0)
					{
						flag=1;
					}
				}
